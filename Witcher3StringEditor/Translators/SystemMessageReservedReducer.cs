...
internal class SystemMessageReservedReducer : IChatHistoryReducer
{
    ...
    public SystemMessageReservedReducer(int maxNonSystemMessages)
    {
        if (maxNonSystemMessages < 0)
            throw new ArgumentException(@"The maximum number of preserved messages cannot be negative",
                nameof(maxNonSystemMessages));
        this.maxNonSystemMessages = maxNonSystemMessages;
    }

    Task<IEnumerable<ChatMessageContent>?> IChatHistoryReducer.ReduceAsync(
        IReadOnlyList<ChatMessageContent> chatHistory, CancellationToken cancellationToken)
    {
